...
confusion_mat = confusionmat(Y_test,prediction) %[confusion_mat,classes]
% update the above parameter based on your calculations
...
%% Error
None_zero_move_in_test=Y_test(Y_test~=0);
None_zero_move_in_pred=prediction(Y_test~=0);
sumn=sum(None_zero_move_in_test~=None_zero_move_in_pred);
error=(sumn/length(None_zero_move_in_test))*100
